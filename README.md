...
1. **Compile the style** (`style.sh` → `compile-carto.js`) — concatenates `style/*.less` into `style.less`, then compiles the CartoLESS into `style.json`, a flat list of paint rules. `color.js` handle CSS color parsing, and `calc.js` evaluates CartoCSS numeric expressions.
